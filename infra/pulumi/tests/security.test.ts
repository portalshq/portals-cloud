...
test("single host uses an Elastic IP lifecycle handler and preserves task IAM isolation", () => {
  ...
  assert.match(compute, /autoScalingGroupName\/\$\{autoScalingGroupName\}/);
  assert.match(compute, /rule\/\$\{lifecycleEventRuleName\}/);
  assert.match(compute, /function:\$\{lifecycleFunctionName\}/);
  assert.match(compute, /memoryEventRuleArns/);
  assert.match(compute, /memoryAlarmArns/);
  assert.doesNotMatch(compute, /events:\*:\*:rule\/\$\{prefix\}-\*/);
  assert.doesNotMatch(compute, /lambda:\*:\*:function:\$\{prefix\}-\*/);
  assert.doesNotMatch(compute, /cloudwatch:\*:\*:alarm:\$\{prefix\}-\*/);
  assert.match(compute, /name: lifecycleFunctionName/);
  assert.match(compute, /name: lifecycleEventRuleName/);

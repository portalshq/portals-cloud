...
import { FrontendServiceArgs } from "../interfaces";

/**
 * FrontendService Component
 * 
 * Creates an ECS Fargate service for Frontend with:
 * - Docker image build and push to ECR
 * - ECS task definition
 * - ECS service with ALB integration
 * - Security groups
 */
export class FrontendService extends pulumi.ComponentResource {
  ...
  constructor(name: string, args: FrontendServiceArgs, opts?: pulumi.ComponentResourceOptions) {
    super("portals:platform:FrontendService", name, {}, opts);

    const resourcePrefix = `${args.projectName}-${args.environment}`;

    // Create security group for Frontend service
    this.securityGroup = new aws.ec2.SecurityGroup(`${resourcePrefix}-frontend-sg`, {
      vpcId: args.vpcId,
      description: "Security group for Frontend service",
      tags: {
        Name: `${resourcePrefix}-frontend-sg`,
    ...
    new aws.ec2.SecurityGroupRule(`${resourcePrefix}-frontend-alb-ingress`, {
      type: "ingress",
      fromPort: 80,
      toPort: 80,
      protocol: "tcp",
    ...
    // Allow egress from Frontend service
    new aws.ec2.SecurityGroupRule(`${resourcePrefix}-frontend-egress`, {
      type: "egress",
      fromPort: 0,
      toPort: 0,
      protocol: "-1",
    ...
    this.dockerImage = new docker.Image(`${resourcePrefix}-frontend-image`, {
      build: {
    ...
    this.taskDefinition = new aws.ecs.TaskDefinition(`${resourcePrefix}-frontend-task`, {
      family: `${resourcePrefix}-frontend`,
      networkMode: "awsvpc",
      ...
      tags: {
        Name: `${resourcePrefix}-frontend-task`,
        Project: args.projectName,
        Environment: args.environment,
        Service: "frontend",
      },
    ...
    // Create ECS service
    this.service = new aws.ecs.Service(`${resourcePrefix}-frontend-service`, {
      ...
      tags: {
        Name: `${resourcePrefix}-frontend-service`,
        Project: args.projectName,
        Environment: args.environment,
        Service: "frontend",
      },

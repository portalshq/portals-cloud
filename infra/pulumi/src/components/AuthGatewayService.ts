...
/** Host-network Auth Gateway: public traffic reaches it only through the ALB. */
  ...
  constructor(name: string, args: AuthGatewayServiceArgs, opts?: pulumi.ComponentResourceOptions) {
    ...
    const task = new aws.ecs.TaskDefinition(`${prefix}-auth-gateway-task`, {
      ...
      containerDefinitions: pulumi.all([
        ...
        name: "auth-gateway", image, essential: true, cpu: Number(args.cpu), memory: Number(args.memory),
        portMappings: [
          { containerPort: 8084, hostPort: 8084, protocol: "tcp", name: "auth-grpc", appProtocol: "grpc" },
          { containerPort: 8085, hostPort: 8085, protocol: "tcp", name: "auth-http", appProtocol: "http" },
          { containerPort: 8086, hostPort: 8086, protocol: "tcp", name: "internal-http", appProtocol: "http" },
          { containerPort: 8087, hostPort: 8087, protocol: "tcp", name: "rebac", appProtocol: "grpc" },
        ],
        secrets: [
        ...
        environment: [
          ...
          { name: "INTERNAL_LISTEN_ADDR", value: "127.0.0.1:8086" },
          { name: "REBAC_LISTEN_ADDR", value: "127.0.0.1:8087" },
    ...
    this.service = new aws.ecs.Service(`${prefix}-auth-gateway-service`, {
      ...
      loadBalancers: args.attachToAlb ? [
        ...(args.publicIngressEnabled ? [{ targetGroupArn: args.grpcTargetGroupArn, containerName: "auth-gateway", containerPort: 8084 }] : []),
        { targetGroupArn: args.httpTargetGroupArn, containerName: "auth-gateway", containerPort: 8085 },
      ] : [],
      healthCheckGracePeriodSeconds: args.attachToAlb ? 90 : undefined,

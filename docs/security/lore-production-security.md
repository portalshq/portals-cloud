...
| Lore `41337` | ECS host port; host SG allows ALB only | ALB only | Plaintext h2c gRPC residual-risk hop |
| Lore `41339` | Loopback/host-local | ECS readiness only | Store-aware container readiness |
...
sends a token or repository payload. The two ALB-to-host hops are currently
unencrypted. The host security group permits their ports only from the ALB;
security groups do not provide cryptographic secrecy.

...
**Current status (2026-08-25 19:46 UTC)**: Production ECR repositories `portals-prod/lore` + `portals-prod/auth-gateway` exist (`IMMUTABLE`, `scanOnPush`). The `prod` Pulumi stack **is deployed** (155 resources, VPC `vpc-087e23a245117cfd5` `10.1.0.0/16`, `lore:8` `c60b9ca368cf…` `HEALTHY` `72bc9186` `v0.8.4-portals.8` `47333fc`, Auth Gateway `5d8e87ce` `HEALTHY`, ALB `portals-prod-alb-fe7ca3e-289037285.us-east-1.elb.amazonaws.com` with `publicIngressEnabled=true` `lore.portals.works:443` + `auth.portals.works:443`). **Fixes since 2026-08-20**: `EgressControls` `VPC endpoints + private zone alias auth→ALB`, Lore SG tightened (`8087→100.64.0.0/10` `VIP` + `VPC`, `443→0.0.0.0/0`, `80` hotfix removed 2026-08-25), `fix(rebac):8087` `3694edb` + `fix(auth):https` `f4ebbe5` (UrcAuthApi via `https://auth.portals.works:443` → `:8084`, RebacApi via `VIP:8087`), `RUST_LOG=debug` one cycle for capture, `preload`-free single-arch build fix (`$BUILDPLATFORM`/`$TARGETPLATFORM`), remote-builder credential hardening. `pulumi up` steady state — `prod` Cognito `us-east-1_IpSPOHKSW` `2oggdt8vf9hihcbtuvdl3tipr4`, KMS JWT key `33e68d77...`, artifact-signing `alias/portals-artifact-signing` `65aee9ea…`, RDS `portals-prod-db` `db.t4g.micro`/`15.18`/1-day+PITR bridge 7× manual snapshots, DynamoDB PITR, S3 versioned. ACM `32f56a6f…` `ISSUED` 2026-08-22 `NotAfter 2027-03-07` (Cloudflare `lore`/`auth.portals.works` → ALB `DNS-only`). `versions.yaml` `lore@sha256:72bc91` `lore-client 0.8.4-portals.8` `nap-client 0.5.15` `Verified OK` (cosign+Trivy 0 high, SBOM/provenance decode). E2E verified 2026-08-25: `create 0.8s` `01a03a7…`, `clone 1.06s`, `stage+commit+push 0.77s` `e881f13…` `43 B` file, `verify-external-surface.sh` `443 open` `8083,41337,41339 closed`. `prod` `ISSUED`, `publicIngressEnabled=true` gated on `release.status: approved` + `securityReviewDate ≤90d`.

...
**Current status (2026-08-25 19:46 UTC)**: See `release-cycle-portals-works.md:0` for the live snapshot — this section is now a pointer, not a second live copy. At `2026-08-25 19:46 UTC` the snapshot read: `prod` `155 resources` `lore:8` `c60b9ca368cf…` `HEALTHY` `72bc9186` `v0.8.4-portals.8` `47333fc`, `ACM 32f56a6f ISSUED`, `versions.yaml Verified OK`, `E2E 0.8s/1.06s/0.77s PASS`. See `rollout-status-2026-08-10.md:Verified Infrastructure State (2026-08-25)` for the dated record; see `release-cycle:4` for phase owners/commands.

# ADR 0003: Non-breaking integration of PX v0 and Narrative Engine v0

## Status
Accepted

## Context
PX v0 and the Narrative Engine v0 (alpha) are already in production,
...
- No changes to studio-app or 25thChapter are required for this repo to
  exist. They keep using PX v0 / the narrative engine directly until the
...
- Third-party channel developers get a stable contract-compliant interface
  immediately for everything *except* PX/narrative-engine calls, which
  remain blocked on the extraction step.
- Whoever owns PX/narrative-engine should treat extracting them into

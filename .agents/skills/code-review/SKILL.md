...
6. **Build before verifying**: Build/regenerate the workspace before calling `architect`, so its checks run against current artifacts instead of a dependency's stale last build (which surfaces phantom errors). TypeScript example: `tsc --build` (or the repo's `typecheck:libs`).

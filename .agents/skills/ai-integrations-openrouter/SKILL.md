...
The callback derives and sets `AI_INTEGRATIONS_OPENROUTER_BASE_URL` and `AI_INTEGRATIONS_OPENROUTER_API_KEY`.
...
```javascript
const openrouterResult = await setupReplitAIIntegrations({
  providerSlug: "openrouter",
});

...
- **Steps**: inline the full recipe below (adapt the wording to fit the user's ask). Don't write `(see guidance elsewhere)` -- put the steps IN the plan. The last two steps should be `presentArtifact({ artifactId })` -- `MarkTaskComplete`, in that order.
...
9. **Call `MarkTaskComplete`.**

Order: verify/screenshot -- `presentArtifact({ artifactId })` -- `MarkTaskComplete`. Once you call `MarkTaskComplete`, the session ends, so any earlier step that didn't happen won't happen anymore.
...
- If your task is the migration phase, read the referenced `port-existing.md` file, port the existing app, ask the user to verify it, and then call `MarkTaskComplete`.

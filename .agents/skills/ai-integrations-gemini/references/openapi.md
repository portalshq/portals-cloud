...
- The `generateGeminiImage` endpoint returns base64 image data. Use the `generateImage` helper from `@workspace/integrations-gemini-ai/image` in the route handler. This is an app helper, not the agent `media-generation` callback.
